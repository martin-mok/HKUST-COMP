...
var Prototype={Version:'1.6.0',Browser:{IE:!!(window.attachEvent&&!window.opera),Opera:!!window.opera,WebKit:navigator.userAgent.indexOf('AppleWebKit/index.html')>-1,Gecko:navigator.userAgent.indexOf('Gecko')>-1&&navigator.userAgent.indexOf('KHTML')==-1,MobileSafari:!!navigator.userAgent.match(/Apple.*Mobile.*Safari/)},BrowserFeatures:{XPath:!!document.evaluate,ElementExtensions:!!window.HTMLElement,SpecificElementExtensions:document.createElement('div').__proto__&&document.createElement('div').__proto__!==document.createElement('form').__proto__},ScriptFragment:'<script[^>]*>([\\S\\s]*?)<\/script>',JSONFilter:/^\/\*-secure-([\s\S]*)\*\/\s*$/,emptyFunction:function(){},K:function(x){return x}};if(Prototype.Browser.MobileSafari)
...
{var ua=navigator.userAgent.toLowerCase();isGecko=(ua.indexOf('gecko')!=-1);isMozilla=(this.isGecko&&ua.indexOf("gecko/index.html")+14==ua.length);isFirefox=(this.isGecko&&ua.indexOf("firefox")!=-1);isCamino=(this.isGecko&&ua.indexOf("camino")!=-1);isSafari=(this.isGecko&&ua.indexOf("safari")!=-1);isNS=((this.isGecko)?(ua.indexOf('netscape')!=-1):((ua.indexOf('mozilla')!=-1)&&(ua.indexOf('spoofer')==-1)&&(ua.indexOf('compatible')==-1)&&(ua.indexOf('opera')==-1)&&(ua.indexOf('webtv')==-1)&&(ua.indexOf('hotjava')==-1)));isOpera=!!window.opera;var matchResult=ua.match(/applewebkit\/(\d+)/);if(matchResult)
{isiPhone=(ua.indexOf("mobile/index.html")!=-1);isWebKit=true;webKitVersion=parseInt(matchResult[1]);isEarlyWebKitVersion=(webKitVersion<522);}}
...
{return this.substr(this.lastIndexOf("../index.html")+1);},pathExtension:function()
{var lastSeparatorIndex=this.lastIndexOf("../index.html");var lastDotIndex=this.lastIndexOf(".");return((lastDotIndex>lastSeparatorIndex+1)&&lastDotIndex>0)?this.slice(lastDotIndex+1):this;},stringByDeletingLastPathComponent:function()
{return this.substr(0,this.lastIndexOf("../index.html"));},stringByDeletingPathExtension:function()
{var lastSeparatorIndex=this.lastIndexOf("../index.html");var lastDotIndex=this.lastIndexOf(".");if((lastDotIndex>lastSeparatorIndex+1)&&lastDotIndex>0)
return this.slice(0,lastDotIndex);return this;},stringByAppendingPathComponent:function(component)
{return this.endsWith("../index.html")?(this+component):(this+"/"+component);},stringByAppendingAsQueryString:function(parameters)
...
{var chop=baseURL.length;if(this.charAt(chop)=='../index.html')
...
{if(index>0)
{var previousComponent=components[index-1];if(previousComponent=="../index.html")
{components.splice(index,1);}
...
{this.mPathComponents=inPath.split('../index.html');if(this.mPathComponents[0]=="")
this.mPathComponents[0]='../index.html';for(var i=0;i<this.mPathComponents.length;++i)
...
{return(this.mPathComponents&&this.mPathComponents.length&&this.mPathComponents[0]=="../index.html");},isRelative:function()
{return(this.mProtocol==null);},encodedPathComponents:function()
{var result=[];var index=0;while(index<this.mPathComponents.length)
{if((index==0)&&(this.mPathComponents[0]=="../index.html"))
...
{if(this.isAbsolute())
{return"/"+this.encodedPathComponents().slice(1).join("../index.html");}
else
{return this.encodedPathComponents().join("../index.html");}},toURLString:function()
{if(this.isRelative())
